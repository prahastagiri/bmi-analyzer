...
/**
 * Trend chart for saved weight & BMI, drawn as a self-contained SVG (no chart
 * library). Includes the healthy zone band (BMI 18,5–24,9 or the matching
 * weight range) and a metric toggle. Shows an encouragement state until the
 * user has saved at least three measurements.
 *
 * @param {{
 *   items: Array<{
 *     created_at: string,
 ...
 */
export function BmiTrendChart({ items }) {
  const [metric, setMetric] = useState(/** @type {"bmi" | "weight"} */ ("bmi"));
