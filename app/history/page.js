...
/**
 * History route that loads saved BMI analyses for the current authenticated user.
 *
...
export default function HistoryPage() {
  ...
  const selectedResult = useMemo(
    () => (selectedInput ? calculateAnalysis(selectedInput) : null),
    [selectedInput]
  );
  ...
  async function handleDetailExport(type) {
    if (!selectedResult) {
...
/**
 * Simple centered message card used for empty, unauthenticated, and disabled
 * states on the history page.
 *
 * @param {{
 ...
 * @returns {import("react").JSX.Element}
 */
function CenterCard({ action, description, title }) {
  return (
...
/**
 * Displays a single value in the history grid.
 *
